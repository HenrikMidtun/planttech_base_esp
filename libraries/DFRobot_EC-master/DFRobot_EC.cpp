...
void DFRobot_EC::ecCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        case 2:
        if(enterCalibrationFlag){
            if((this->_rawEC>RAWEC_1413_LOW)&&(this->_rawEC<RAWEC_1413_HIGH)){                         //recognize 1.413us/cm buffer solution
                compECsolution = 1.413*(1.0+0.0185*(this->_temperature-25.0));  //temperature compensation
            }else if((this->_rawEC>RAWEC_1288_LOW)&&(this->_rawEC<RAWEC_1288_HIGH)){                    //recognize 12.88ms/cm buffer solution
